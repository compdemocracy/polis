    ...
    routes: inventory.routes
      ...
      .flatMap((r, i) =>
        Array.from({ length: r.method === "ALL" ? 35 : 1 }, (_, j) => ({
          ...
          method:
            r.method === "ALL" ? require("./core.cjs").allMethods[j] : r.method,
          path: r.path,
        }))
      ),
  };
...
test("route dump cannot run before readiness", () =>
  assert.throws(
    () => normalizeDump({ ...dump(), ready: false }, inventory),
    /readiness/
  ));
...
test("same path ALL registrations stay distinct", () =>
  assert.equal(
    normalizeDump(dump(), inventory).filter((r) => r.path === "/api/v3/*")
      .length,
    2
  ));
test("seeded serialized case generation is reproducible and seed-dependent", () => {
  assert.equal(
    canonical(generate(inventory, scope)),
    canonical(generate(inventory, scope))
  );
  assert.notEqual(
...
test("delete a recording or auth mode: coverage turns red", () => {
  const cs = generate(inventory, scope).map((c) => ({
    ...c,
    routeHits: [1, 3],
  }));
...
test("row delta retains duplicates and distinguishes updates", () =>
  assert.deepEqual(
    delta({ t: [{ vote: -1 }, { vote: -1 }] }, { t: [{ vote: 1 }] }),
    {
...
const original = {
  ...
  effects: {
    db: { comments: { added: [{ txt: "generated" }] } },
    outbound: [{ service: "SES" }],
  },
  config: { runtime: "node" },
};
for (const [name, change, expected] of [
  ...
  [
    "remove page two",
    (x) => x.response.body.pages.pop(),
    "$.response.body.pages.1",
  ],
  [
  ...
  [
    "alter runtime metadata",
    (x) => (x.config.runtime = "different"),
    "$.config.runtime",
  ],
  [
    "truncate a stream",
    (x) => (x.response.body.stream = "comp"),
    "$.response.body.stream",
  ],
])
  test(`differ unit example (not recorded gate): ${name}`, () => {
...
test("process errors fail even after successful response, including swallowed 23505", () =>
  assert.equal(
    oracle({ completed: true }, [
      { event: "unhandledRejection", code: "23505" },
    ]).pass,
    false
  ));
...
test("portable per-case entropy replays exact draws and differs by seed", () => {
  ...
  a.seed("000000010000");
  b.seed("000000010000");
  assert.equal(a.next(), 270369 / 4294967296);
  assert.equal(b.next(), 270369 / 4294967296);
  assert.deepEqual(
...
test("URL capability normalization keeps path/ID consistency and differences", () => {
  const a = new Normalizer(),
    b = new Normalizer();
  assert.deepEqual(
    a.normalize({
      conversation_id: "2abc",
      url: "https://example.invalid/2abc",
    }),
    b.normalize({
      conversation_id: "2def",
      url: "https://example.invalid/2def",
    })
  );
  assert.notEqual(
    a.normalize({ url: "https://example.invalid/wrong" }).url,
    a.normalize({ url: "https://example.invalid/2abc" }).url
  );
});
test("encoded PCA JSON uses the same clock rules and rejects truncated gzip", () => {
  const { gzipSync } = require("node:zlib");
  const a = { lastVoteTimestamp: 1700000000001, vote: -1 },
    b = { lastVoteTimestamp: 1700000000002, vote: -1 };
  const encode = (x) => ({
    type: "Buffer",
    data: [...gzipSync(JSON.stringify(x))],
  });
  const n = new Normalizer();
  assert.deepEqual(
    n.normalize(encode(a), "$.response.body.pca.asBufferOfGzippedJson"),
    n.normalize(encode(b), "$.response.body.pca.asBufferOfGzippedJson")
  );
  assert.notDeepEqual(
    n.normalize(encode(a), "$.response.body.pca.asBufferOfGzippedJson"),
    n.normalize(
      encode({ ...b, vote: 1 }),
      "$.response.body.pca.asBufferOfGzippedJson"
    )
  );
  const bad = encode(a);
  bad.data.pop();
  assert.throws(() =>
    n.normalize(bad, "$.response.body.pca.asBufferOfGzippedJson")
  );
});

test("ALL rejects an unsupported verb even when there are still 35 distinct entries", () => {
  const d = dump();
  d.routes[0].method = "BOGUS";
  assert.throws(() => normalizeDump(d, inventory), /fan-out/);
});
