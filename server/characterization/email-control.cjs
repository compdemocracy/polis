...
async function main() {
  ...
  if (notify.isNotificationLoopStarted())
    throw Error("notification loop started on import");
  ...
  const timer = setInterval(() => {
    ...
    const blocked = s.outbound.find(
      (x) => x.blocked && x.host === "email.us-east-1.amazonaws.com"
    );
    if (command && blocked) {
      clearInterval(timer);
    ...
    if (performance.now() > deadline) {
      console.error(
        "FAIL: real notification loop did not produce a blocked SES attempt"
      );
      process.exit(1);
    }
  }, 100);
