...
function generate(inventory, scope, seed = 'p027-v1', profile = 'boundary') {
  ...
  for (const r of inventory.routes) {
    ...
    if(r.method==='ALL') {
      if(r.id===3)for(const auth of modes)cases.push({
        caseId:`r3/${auth}/options`,routeId:3,auth,case:'options',seed:hash(`${seed}/3/${auth}/options`).slice(0,12),
        request:{method:'OPTIONS',path:'/api/v3/p027-options',query:{},headers:{accept:'application/json',origin:'https://example.invalid','x-forwarded-proto':'https',...(auth!=='unauthenticated'?{authorization:`$auth:${auth}`}:{})},body:null}
      });
      continue;
    }
    for (const auth of modes) {
      const routeSeed = hash(`${seed}/${r.id}/${auth}`).slice(0, 12);
      const path = (r.id === 126 ? '/polis_site_id.generated' : r.path).replace(/:([\w]+)/g, (_, key) =>
        key === 'reportId' || key === 'report_id'
          ...
          : (key === 'zid' || (profile === 'nominal' && ['pmqid','pmaid'].includes(key)))
          ? '1'
          : `generated-${routeSeed}`
      );
      const request = {
        ...
        headers: {
          ...
          ...(auth !== 'unauthenticated' ? { authorization: `$auth:${auth}` } : {}),
        },
        body: null,
      };
      ...
      if (profile === 'nominal') {
        ...
        const extras = {
          ...
          99: ['conversation_id'],
          102: ['conversation_id'],
          107: ['topic', 'is_active', 'is_draft', 'profanity_filter', 'spam_filter'],
        };
        for (const k of extras[r.id] || []) fields[k] = values[k];
        if (r.id === 22 || r.id === 23) fields.signature = 'generated-invalid-signature';
      ...
      if(profile==='nominal' && [11,88,102].includes(r.id)) {
        const name=r.id===11?'invalid-uuid':r.id===88?'empty-update':'missing-conversation';
        const bad=structuredClone(request);
        if(r.id===11)bad.path=`/api/v3/xid/generated-${routeSeed}-xid.csv`;
        else bad.body={};
        cases.push({caseId:`r${r.id}/${auth}/${name}`,routeId:r.id,auth,case:name,
          seed:hash(`${seed}/${r.id}/${auth}/${name}`).slice(0,12),request:bad});
      }
    }
  }
  cases.push({
