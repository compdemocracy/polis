...
function files(dir = process.cwd()) {
  ...
  function visit(p) {
    for (const e of fs.readdirSync(p, { withFileTypes: true })) {
      if (['node_modules', 'dist', '.git', 'characterization', 'source'].includes(e.name)) continue;
      const f = `${p}/${e.name}`;
...
async function main() {
  ...
  const boot = {
    ...
    notificationLoop: require('../src/routes/notify.ts').isNotificationLoopStarted(),
  };
  ...
  http
    .createServer(async (req, res) => {
      ...
      try {
        ...
        const seed=new URL(req.url,'http://localhost').searchParams.get('seed');
        if(!/^[a-f0-9]{12}$/.test(seed||''))throw Error('invalid case entropy seed');
        state.setSeed(seed);return res.end('{}');
      }
      if (req.url === '/ready')
          return res.end(
            JSON.stringify({
        ...
        if (req.url === '/tokens') {
          ...
          const participant = require('../src/auth/anonymous-jwt.ts').issueAnonymousJWT(
            '2p027generated',
            3,
            2
          );
          async function oidc(username) {
            const response = await fetch('https://oidc-simulator:3000/oauth/token', {
              method: 'POST',
            ...
            if (!response.ok) throw Error(`OIDC simulator token failed: ${response.status}`);
        ...
        if (
          ...
          const route = app.routes.get.find((r) => r.path === '/api/v3/testConnection');
          const behavior=req.url;
        route.callbacks[route.callbacks.length - 1] = (request,response) => {
          if(behavior==='/hang-route')return;
          if(behavior==='/exit-route') {setImmediate(()=>{throw new Error('P027_NEGATIVE_EXIT');});return;}
          response.json({generated:true});
          Promise.reject(new Error('P027_NEGATIVE_PROCESS_ERROR'));
        };
        return res.end('{}');
        }
        if (req.url === '/drop-route' && process.env.P027_NEGATIVE_CONTROLS === '1') {
          app.routes.get = app.routes.get.filter((r) => r.path !== '/api/v3/testConnection');
          return res.end('{}');
        }
        res.statusCode = 404;
        res.end('{}');
