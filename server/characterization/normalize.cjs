...
const policy = {
  ...
  encodedFields: {
    "$.response.body.pca.asJSON": "json",
    "$.response.body.pca.asBufferOfGzippedJson": "gzip-json",
  },
  timeFields: [
    "expiration",
    "lastModTimestamp",
...
class Normalizer {
  ...
  normalize(x, path = "$", key = "") {
    ...
    if (encoding) {
      ...
      if (encoding === "gzip-json") {
        if (x.type !== "Buffer" || !Array.isArray(x.data))
          throw Error("invalid PCA gzip buffer");
        decoded = require("node:zlib")
          .gunzipSync(Buffer.from(x.data))
          .toString("utf8");
      } else decoded = x;
      ...
      return {
        $encoding: encoding,
        value: this.normalize(parsed, path + ".decoded"),
      };
    }

    if (
      /^\$\.response\.body(?:\.|\[)/.test(path) &&
      [
        "lastVoteTimestamp",
        "lastModTimestamp",
        "serverTimestamp",
    ...
    if (
      ...
      const verified = require("./jwt.cjs").verifyToken(x, this.binding);
      this.issued.push(verified);
      return this.normalize(verified, path + ".verified");
    }
    ...
    if (policy.urlFields.includes(key) && typeof x === "string") {
      try {
        ...
        if (changed) {
          ...
          this.rules.push({ path, rule: "URL capability path segment" });
          return u.toString();
        }
      } catch {}
      return x;
    ...
    if (policy.symbolFields.includes(key) && typeof x === "string") {
      ...
      if (!this.symbols.has(id))
        this.symbols.set(
          id,
          `$${ns}:${
            [...this.symbols.keys()].filter((k) => k.startsWith(ns + ":"))
              .length + 1
          }`
        );
    ...
    if (Array.isArray(x))
      return x.map((v, i) => this.normalize(v, `${path}[${i}]`, key));
    if (typeof x === "object")
