...
const serverPort = parseInt(
  process.env.API_SERVER_PORT || "5000",
  10
) as number;
...
export default {
  ...
  googleCredentialsBase64:
    process.env.GOOGLE_CREDENTIALS_BASE64 || (null as string | null),
  ...
  shouldUseTranslationAPI: isTrue(
    process.env.SHOULD_USE_TRANSLATION_API
  ) as boolean,
  staticFilesAdminPort: parseInt(
    process.env.STATIC_FILES_ADMIN_PORT || "8080",
    10
  ) as number,
  staticFilesClientPort: parseInt(
    process.env.STATIC_FILES_CLIENT_PORT || "8080",
    10
  ) as number,
