...
// fb gets weird with TypeScript when imported as a module, using require instead.
const FB = require("fb");
...
Bluebird.onPossiblyUnhandledRejection(function (err: { stack: any }) {
...
function doPolisLtiTokenHeaderAuth(
  ...
  const token = req?.headers?.["x-polis"];
...
function initializePolisHelpers() {
  ...
  function handle_GET_launchPrep(req: any, res: any) {
  ...
  */
  ...
  function handle_GET_math_correlationMatrix(
    ...
    Promise.all([resultExistsPromise, getZidForRid(rid)])
      .then((a: any[]) => {
        ...
        const zid = a[1];
        if (!rows || !rows.length) {
          return requestExistsPromise.then((requests_rows: any) => {
  ...
  function handle_POST_auth_password(
    ...
    getUidForPwResetToken(pwresettoken, function (err: any, userParams: any) {
      if (err) {
        console.error(err);
        fail(
          res,
...
<p>Please return to the 'setup pol.is' assignment to sign in as another user.</p>
    ...
    return new Promise(function (
      ...
      generateToken(
        ...
        function (err: any, longStringOfTokens?: string) {
          ...
          const otzinviteArrayRegexMatch = longStringOfTokens?.match(
            /.{1,31}/g
          );
  ...
  function createXidEntry(xid: any, owner: any, uid?: any) {
    return new Promise(function (resolve: any, reject: (arg0: Error) => void) {
      pgQuery(
  ...
  function isConversationOwner(
    ...
    pgQuery_readOnly(
      "SELECT * FROM conversations WHERE zid = ($1) AND owner = ($2);",
      [zid, uid],
      function (err: number, docs: { rows: any }) {
...
${message}`;
  ...
  function sendPasswordResetEmail(
    ...
    getUserInfoForUid(uid, function (err: any, userInfo: any) {
      if (err) {
        return callback?.(err);
      }
      if (!userInfo) {
...
Email verified! You can close this tab or hit the back button.
  ...
  function getDbTime() {
    return pgQueryP("select now_as_millis();", []).then((rows: any) => {
...
<a href="${createNotificationsSubscribeUrl(
  ...
  function handle_POST_auth_login(req: any, res: any) {
    ...
    function doFbNoUserExistsYet(user: any) {
      ...
      if (uid) {
        ...
        const query =
          ...
          "($1, $2) " +
          "returning *;";
        promise = pgQueryP(query, [email, hname]).then(function (rows: any) {
  ...
  function changePlan(uid?: any, planCode?: any) {
    return new Promise(function (resolve: any, reject: (arg0: any) => void) {
      pgQuery(
  ...
  function updateStripePlan(
    ...
    const customerPromise = user.stripeCustomerId
  ...
  function handle_POST_stripe_cancel(
    ...
    getUserInfoForUid2(uid).then((user: any) => {
      ...
      return pgQueryP("select * from stripe_subscriptions where uid = ($1);", [
        ...
        if ("rows" in response || "err" in response) {
          const { rows, err } = response;
          (rows: any, err: any) => {
            if (!rows || !rows.length) {
              return fail(
                res,
            ...
            stripe.customers.cancelSubscription(
              ...
              (err: any) => {
                ...
                return updatePlan(req, res, uid, planCodes.free).then(
                  function () {
                    res.json({});
                  }
                );
  ...
  function handle_POST_charge(
    ...
    const stripeToken = req.p.stripeToken;
    ...
    const updateStripePromise = Promise.resolve();
    if (plan !== "pp") {
  ...
  */
  ...
  function getConversationHasMetadata(zid: any) {
    return new Promise(function (
      ...
      pgQuery_readOnly(
        "SELECT * from participant_metadata_questions where zid = ($1)",
        [zid],
        function (err: any, metadataResults: { rows: any }) {
          if (err) {
  ...
  function getConversationTranslationsMinimal(zid: any, lang: any) {
    ...
    return getConversationTranslations(zid, lang).then(function (rows: any) {
  ...
  function getConversations(
    ...
    const want_inbox_item_participant_url =
      req.p.want_inbox_item_participant_url;
  ...
  function createReport(zid: any) {
    return generateTokenP(20, false).then(function (report_id: any) {
...
`;
  ...
  function switchToUser(req: any, res: any, uid?: any) {
    return new Promise(function (resolve: any, reject: (arg0: string) => void) {
      startSession(uid, function (errSess: any, token: any) {
        if (errSess) {
          reject(errSess);
          return;
  ...
  function handle_GET_ptptois(
    ...
    const socialPtptsPromise = convPromise.then((conv: any) => {
  ...
  function renderLtiLinkagePage(
    ...
    const context_id = req.p.context_id;
    const user_id = req.p.user_id;
    const user_image = req.p.user_image;
    const tool_consumer_instance_guid = req.p.tool_consumer_instance_guid;
...
*/
  ...
  function handle_POST_lti_setup_assignment(
    ...
    const dataSavedPromise = pgQueryP(
  ...
  function handle_POST_lti_conversation_assignment(
    ...
    const isInstructor = /[iI]nstructor/.exec(roles); // others: Learner
    ...
    function constructConversationUrl(zid: any) {
      ...
      return getZinvite(zid).then(function (zinvite: any) {
        return (
          getServerNameWithProtocol(req) +
          "/" +
          zinvite +
    ...
    Promise.all([
      ...
      .then(function (results: any[]) {
        ...
        const exists = infos && infos.length;
        const info = infos[0];
  ...
  function handle_GET_canvas_app_instructions_png(req: any, res: any) {
  ...
  function fetchIndexForConversation(req: any, res: any) {
