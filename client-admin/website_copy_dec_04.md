...
* **Dynamic Opinion Mapping:** The Polis algorithm groups participants based on the statements they submit and how they vote on others' statements, dynamically adjusting opinion clusters as the conversation evolves to maintain coherent analysis across hundreds of thousands of statements and millions of votes.  
* **Semantic Topic Clustering:** Polis 2.0 uses the Embedding Vector Oriented Clustering (EVōC) library from the Tutte Institute for Mathematics and Computing to automatically organize conversations into evolving topic hierarchies—hundreds of topics and subtopics—derived from both organizer-seeded comments and participant input. Participants can view all topic areas and select those of greatest interest before entering the discussion. This organic process enables participants to collectively shape the agenda over time, with "hot" and "cold" areas of discussion naturally emerging, allowing Polis 2.0 conversations to remain open indefinitely.  
...
* **Vote on others’ statements**—agree, disagree, or pass (there’s no reply function)  
