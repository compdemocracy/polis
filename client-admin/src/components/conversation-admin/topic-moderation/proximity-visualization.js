...
class ProximityVisualization extends React.Component {
  ...
  async loadProximityData() {
    try {
      ...
      const response = await fetch(`/api/v3/topicMod/proximity?report_id=${conversation_id}&layer_id=${selectedLayer}`)
  ...
  renderVisualization() {
    ...
    if (xValues.length === 0 || yValues.length === 0) {
      svgElement.innerHTML = '<text x="50%" y="50%" text-anchor="middle" fill="#666">No coordinate data available</text>'
      return
    }
    ...
    const getColor = (status) => {
      switch (status) {
        case 'accepted': case 1: return '#22c55e'
        case 'rejected': case -1: return '#ef4444'
        case 'meta': case 0: return '#f59e0b'
    ...
    Object.entries(clusters).forEach(([clusterId, points]) => {
      ...
      const maxRadius = Math.max(...points.map(p => 
        Math.sqrt(Math.pow(xScale(p.umap_x) - centerX, 2) + Math.pow(yScale(p.umap_y) - centerY, 2))
      )) + 20
      ...
      <Box>
        ...
        {proximityData.length > 0 ? (
          <Box>
            ...
            <Box sx={{ border: '1px solid', borderColor: 'border', borderRadius: 'default', overflow: 'hidden' }}>
              <svg 
