  ...
  renderStatCard(title, value, color = 'primary') {
    return (
      <Card sx={{ p: 3, textAlign: 'center', minWidth: '150px' }}>
        <Text sx={{ fontSize: 3, fontWeight: 'bold', color: color }}>
          {value}
        </Text>
        <Text sx={{ fontSize: 1, color: 'textSecondary', mt: 1 }}>
          {title}
        </Text>
      </Card>
    )
      ...
      <Box>
        ...
        <Box sx={{ mt: 4 }}>
          ...
          <Box sx={{ bg: 'muted', borderRadius: 'default', p: 3 }}>
            ...
            <Box sx={{ bg: 'background', borderRadius: 'default', overflow: 'hidden', height: '20px' }}>
              <Flex sx={{ height: '100%' }}>
                <Box 
                  sx={{ 
                    bg: 'green', 
