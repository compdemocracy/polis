...
# Paths.
# ---------------------------------------------------------------------------
# Cross-tree anchors (see polismath.paths). _MATH_ROOT is the clj ORACLE
# tree — removed from the working tree at cutover Step #4; re-replaying the
# oracle needs a git-history checkout of math/ (the requires_math_tree test
# guards skip oracle-dependent paths when dev/replay.clj is absent).
_DELPHI_ROOT = paths.DELPHI_ROOT
_REPO_ROOT = paths.REPO_ROOT
_MATH_ROOT = paths.CLJ_TREE_ROOT
...
def _manifest_matches_inputs(manifest_path: Path, input_keys: dict[str, Any]) -> bool:
    """Subset match on the INPUT cache keys only (votes/schedule/comments) —
    the oracle-absent path of :func:`ensure_clj_recording`, where the stored
    clj-source hashes cannot be recomputed for comparison."""
    if not manifest_path.exists():
        return False
    try:
        existing = json.loads(manifest_path.read_text())
        ...
        return False
    return all(existing.get(k) == v for k, v in input_keys.items())
...
#: Default root for :func:`engine_tree_hash` — the installed package tree.
#: Module-level so tests can monkeypatch the seam (pre-Step-#4 they patched
#: ``_DELPHI_ROOT`` and relied on the root being derived from it).
...
def engine_tree_hash(polismath_root: str | Path | None = None) -> str:
    ...
    root = Path(polismath_root) if polismath_root is not None else _ENGINE_TREE_ROOT
...
def _raise_missing_oracle() -> None:
    raise CertifyError(
        "clj-oracle",
        "Clojure oracle tree not present at math/ (removed at cutover "
        "Step 4; it lives in git history). To RE-REPLAY the oracle, "
        "restore it from a pre-Step-4 revision — recipe in math/README.md "
        "('certification oracle'). Cached pairs need no oracle; only a "
        "cache miss or --refresh forces a clj re-replay.",
    )
...
def _clj_source_hashes() -> tuple[str, str]:
    ...
    Post-cutover (Step 4) the Clojure oracle tree is absent from the
    working tree — fail with instructions rather than a bare
    FileNotFoundError (paths.py CLJ_TREE_ROOT's contract)."""
    if not (_MATH_ROOT / "dev" / "replay.clj").exists():
        _raise_missing_oracle()
    return (
...
def ensure_clj_recording(
    ...
    if comments_csv is not None:
        input_keys["comments_csv_sha256"] = sha256_file(comments_csv)
    if not (_MATH_ROOT / "dev" / "replay.clj").exists():
        # Post-cutover-Step-4: the oracle tree lives only in git history.
        # A cached recording whose INPUT keys (votes/schedule/comments)
        # match is trusted as-is — its stored clj-source hashes describe
        # the tree it was recorded against, which cannot be (and need not
        # be) revalidated without the tree. Only an actual re-replay —
        # cache miss or --refresh — demands the restored oracle.
        if not refresh and _manifest_matches_inputs(manifest_path, input_keys):
            return clj_dir, True
        _raise_missing_oracle()
