...
# Cross-tree anchors (see polismath.paths — the test estate stays under
# delphi/; the clj oracle tree exists only in git history post-cutover).
# Module-level names kept: tests monkeypatch/assert on them.
...
def build_py_env(
    *,
    ...
    """Env for ``python -m polismath.poller``, keyed to ``PollerConfig.from_env``
    ...
    (2026-07-24 live debug): the poller entry never loads a `.env`
...
class PyPollerRunner(_SubprocessRunner):
    """The python math poller (``python -m polismath.poller`` — was
    ``scripts/math_poller.py`` before cutover Step 4), run-forever mode —
    its only flag is ``--once`` (polismath/poller/__main__.py), which we do
    NOT pass, so it runs until killed exactly like the clj container.
    cwd stays ``_DELPHI_ROOT``: that is where the uv project/venv lives."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ["uv", "run", "python", "-m", "polismath.poller"], cwd=_DELPHI_ROOT, env=env,
            log_path=log_path,
        )
