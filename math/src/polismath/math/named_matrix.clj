...
(ns polismath.math.named-matrix
  (:require
   ...
   [polismath.utils :refer [filter-by-index]]))
...
(deftype IndexHash
  ...
  AutoIncIndex
  (get-names [_this] (.names _this))
  (next-index [_this] (count (.names _this)))
  (index [_this keyname] (index-hash keyname))
  (append [this keyname]
...
(deftype NamedMatrix
  ...
  (update-nmat [this values]
    ...
    (let [[missing-rows missing-cols]
          (reduce
           (fn [[missing-rows missing-cols] [row col _value]]
...
(s/fdef whatever
  :args (s/cat :x (s/and number? #(not= ##NaN %) #(not (zero? %))))
  :ret number?
  :fn #(= (-> % :ret (* 2)) (-> % :args :x)))
(defn whatever
  [x]
