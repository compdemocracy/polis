...
(ns ^{:clj-kondo/ignore [:unresolved-symbol]}
 polismath.math.repness
  (:refer-clojure :exclude [* - + == /])
  (:require
   [clojure.core.matrix :as matrix]
   [clojure.core.matrix.operators :refer [* + - / max]]
...
(defn finalize-cmt-stats
  ...
  [tid {:keys [na nd ns pa pd pat pdt ra rd rat rdt]}]
...
(defn extremeness
  ...
  (matrix/dot (- ptpt-position group-center)
        ...
        (->> group-clusters
             (map (fn [{:keys [id members]}]
                    (let [pids (apply concat (map bid->pid members))
                          ...
                          extreme-direction (matrix/normalise (- center global-center))]
