...
const App = (props) => {
  ...
  const getMath = async (conversation_id, authToken = null) => {
    const data = await net.polisGet("/api/v3/math/pca2", {
      lastVoteTimestamp: 0,
      conversation_id,
    }, authToken);
  ...
  const getReport = async (report_id, authToken = null) => {
    const reports = await net.polisGet("/api/v3/reports", {
      report_id,
    }, authToken);
  ...
  const getCorrelationMatrix = async (math_tick, authToken = null) => {
    try {
      const response = await net.polisGet("/api/v3/math/correlationMatrix", {
        math_tick,
        report_id,
      }, authToken);
      ...
      if (response.status && response.status === "pending") {
        ...
        await new Promise((resolve) => {
          setTimeout(resolve, corMatRetries < 10 ? 200 : 3000);
        });
        ...
        throw new Error("Currently, No comments are selected for display in the matrix.");
      } else {
        return response;
      }
    } catch (err) {
  ...
  const getData = async (authToken = null) => {
    try {
      const _report = await getReport(report_id, authToken);
      
      // Start parallel requests that depend on report
      const [mathResult, _conversation] = await Promise.all([
        getMath(_report.conversation_id, authToken),
        getConversation(_report.conversation_id, authToken),
      ]);
      ...
      const [commentsRaw, _participants, correlationHClust] = await Promise.all([
        ...
        globals.enableMatrix
          ? getCorrelationMatrix(mathResult.math_tick, authToken)
          : Promise.resolve(null),
      ]);

      // Handle narrative separately (it's fire-and-forget)
      if (isNarrativeReport) {
        getNarrative(_report.report_id, authToken).catch((err) => {
  ...
  useEffect(() => {
    const init = async () => {      
      ...
      try {
        const response = await net.polisGet("/api/v3/delphi", {
          report_id,
        }, token);
