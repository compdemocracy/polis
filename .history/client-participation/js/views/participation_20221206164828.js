...
module.exports = ConversationView.extend({
  ...
  updateVoteRemaining: function () {
    if (useVoteMoreBlocker) {
      this.voteMoreModel.set(
        "remaining",
        Math.max(0, 2 - this.votesByMe.length)
      );
    }
  },
  ...
  context: function () {
    ...
    ctx.auth_prompt_any_social =
      ctx.auth_opt_fb_computed || ctx.auth_opt_tw_computed;
  ...
  */
    ...
    if (
      /^ *$/.test(ctx.description) ||
      _.isNull(ctx.description) ||
      ctx.description === ""
    ) {
    ...
    ctx.no_vis =
      !Utils.userCanSeeVis() || ctx.vis_type === Constants.VIS_TYPE.OFF;
    ctx.no_write =
      ctx.write_type === 0 || !Utils.userCanWrite() || !ctx.is_active;
    ctx.no_voting = !Utils.userCanVote() || !ctx.is_active;
    ctx.no_topic =
      !Utils.userCanSeeTopic() || !ctx.topic || ctx.topic.length === 0;
    ctx.no_description =
      !Utils.userCanSeeDescription() ||
      !ctx.description ||
  ...
  twitterConnectBtn: function () {
    ...
    window.open(
      document.location.origin +
        "/api/v3/twitterBtn?dest=/twitterAuthReturn/ParticipationView",
      "twitterWindow",
      params
    );
  ...
  updateTopComments: function () {
    ...
    _.each([topComments, divisiveComments], function (collection) {
      _.each(collection, function (c) {
        ...
        c.set(
          "percentPassed",
          Math.round((100 * (gv.saw - gv.disagreed - gv.agreed)) / gv.saw)
        );
  ...
  shouldShowVisUnderTabs: function () {
    return (
      ...
      (this.conversationTabs.onAnalyzeTab() ||
        this.conversationTabs.onGroupTab())
    );
  },
  ...
  updateVisibilityOfSocialButtons: function () {
    ...
    var votedEnough =
      (this.readReactModel &&
        this.readReactModel.get("voteCount") >= voteCountForFacebookPrompt) ||
      false;
  ...
  doInit: function (options, firstComment) {
    ...
    function onPersonUpdate(updatedNodes, newClusters, newParticipantCount) {
      ...
      newGroups.push({
        name: display.xs()
          ? Strings.majorityOpinionShort
          : Strings.majorityOpinion,
        styles: "margin-left: 20px;",
        gid: -1,
      });
      that.groupNamesModel.set("groups", newGroups);

      $(".participationCount").html(
        newParticipantCount +
          (newParticipantCount === 1 ? " person" : " people")
      );
      that.updateVis2();
      that.updateTopComments();
    }
    ...
    function initPcaVis() {
      ...
      vis = that.vis = new VisView({
        ...
        getParticipantsOfInterestForGid:
          serverClient.getParticipantsOfInterestForGid,
        xOffset: xOffset,
        w: w,
        h: h,
