    ...
    for key in structure["keys"]:
        if key not in ["comment-priorities", "group-clusters"]:
            ...
            if isinstance(value, dict):
                ...
                structure[key] = {"type": "list", "length": len(value), "sample": value[:2] if value else []}
            else:
                structure[key] = {"type": type(value).__name__, "value": value}
    ...
    for dataset, result in results.items():
        if dataset == "biodiversity":
            output_dir = os.path.abspath(
                os.path.join(os.path.dirname(__file__), "..", "real_data/biodiversity/python_output")
            )
        else:
