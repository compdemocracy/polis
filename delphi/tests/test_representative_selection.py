"""Public-fixture-only snapshot selection, disclosure boundary and extraction wiring.
...
def configured():
    ...
    config["config_version"] = "public-fixture-representative-v1"
...
def _cli():
    ...
    spec = importlib.util.spec_from_file_location("public_fixture_selection_cli", path)
...
def test_permuted_population_and_unrelated_outcomes_do_not_change_selection():
    ...
    variant = [dict(row, outcome="FAIL", topic="PUBLIC_FIXTURE_PRIVATE_TEXT") for row in reversed(rows)]
    assert sel.select_representative(variant, SEED) == expected
    assert rows == before
    assert "PUBLIC_FIXTURE_PRIVATE_TEXT" not in json.dumps(expected.report)
...
def test_report_allowlist_rejects_every_unknown_field(level, forbidden):
    ...
    target[forbidden] = "PUBLIC_FIXTURE_PRIVATE_VALUE"
    with pytest.raises(sel.SelectionError) as exc:
...
def test_selection_only_reads_one_snapshot_without_resolving_recipes(monkeypatch):
    ...
    result=fx.select_representative_from_config(object(),config=configured(),snapshot_id='public-fixture-snapshot',writers_disabled=True)
    assert calls==[('open',{'snapshot_id':'public-fixture-snapshot','writers_disabled':True}),('fetch',None)]
    assert len(result['provenance_rows'])==20
    assert result['report']['bucket_counts']['population']==22
    assert result['transaction_guarantee']['imported_snapshot_id']=='public-fixture-snapshot'
...
def test_whole_extraction_shares_census_and_preserves_existing_recipe_resolution(tmp_path,monkeypatch,enabled):
    ...
    monkeypatch.setattr(fx,'detect_tie_key',lambda conn:{'guarantee':'public-fixture'})
    ...
    result=fx.extract_from_config(object(),config=config,payload_root=tmp_path,guard_root=tmp_path,snapshot_id='public-fixture')
...
def test_selection_cli_outputs_only_report_and_stores_private_mapping(tmp_path,monkeypatch,n):
    ...
    assert conn.closed
    assert 'zid' not in result.output and 'public-fixture-snapshot' not in result.output and str(tmp_path) not in result.output
...
def test_cli_errors_never_render_private_details(tmp_path,monkeypatch,failure):
    ...
    if failure=='database':
        def explode(*a):raise RuntimeError('PUBLIC_FIXTURE_PRIVATE_CONNECTION')
        monkeypatch.setattr(cli.psycopg2,'connect',explode)
    if failure=='outside_path':output=tmp_path/'PUBLIC_FIXTURE_PRIVATE_PATH'
...
def test_from_config_cli_confines_selection_identity_to_its_private_sidecar(tmp_path,monkeypatch):
    ...
    result={'survey':{'private_zid':910000001,'n_conversations':25},
            'provenance_rows':[{'zid':910000002,'role':'public-fixture-old-role'}],
            'roles':[], 'generated':[], 'dir_names':{}, 'coverage_report':{},
            'transaction_guarantee':{'imported_snapshot_id':'public-fixture-private-snapshot'},
    ...
    assert 'public-fixture-private-snapshot' not in output.output
...
def test_from_config_selection_failure_is_fixed_text(tmp_path,monkeypatch,failure):
    ...
    class Conn:
        def close(self):pass
    monkeypatch.setattr(cli.psycopg2,'connect',lambda *a:Conn())
    def explode(*a,**kw):raise ValueError('PUBLIC_FIXTURE_PRIVATE_DETAIL')
    if failure=='path':payload=tmp_path/'PUBLIC_FIXTURE_PRIVATE_PATH'
