...
def timeit_decorator(method_name):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            # Log start
            detail = {"method": method_name, "start_time": start_time, "context": context, "status": "started"}
...
def instrument_conversation_class():
        ...
        for batch_start in range(0, total_votes, batch_size):
            ...
            for vote in batch_votes:
                try:
                    ...
                    result.raw_rating_mat = result.raw_rating_mat.update(ptpt_id, comment_id, vote_value)
                    vote_count += 1
                    ...
                    if sub_step_time > 0.1:  # Log only unusually slow updates
                        elapsed = time.time() - profile_data.get("process_start_time", start_time)
                        print(
                            f"[{elapsed:.2f}s] Slow matrix update for pid={ptpt_id}, tid={comment_id}: {sub_step_time:.4f}s"
                        )
    ...
    NamedMatrix.update = detailed_matrix_update
...
def print_profiling_summary():
    ...
    methods_by_time = sorted(profile_data["method_times"].items(), key=lambda x: x[1], reverse=True)
    ...
    if "slow_matrix_updates" in profile_data and profile_data["slow_matrix_updates"]:
        ...
        slow_updates = sorted(profile_data["slow_matrix_updates"], key=lambda x: x["time"], reverse=True)
