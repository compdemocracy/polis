...
def test_consensus_and_silhouette_are_tolerant_family():
    # P6f: top-level consensus + group-clusters silhouette float divergences
    # belong to the TOLERANT family (soft signals), not exact.
    cmp = sc.StepComparer()
    a = _blob()
    a["consensus"] = {"agree": {"c0": 0.10}}
    a["group-clusters"] = [
        {"id": 0, "center": [0.5, 0.5], "members": [1, 2], "silhouette": 0.70}
    ]
    b = copy.deepcopy(a)
    b["consensus"]["agree"]["c0"] = 0.95              # top-level consensus float
    b["group-clusters"][0]["silhouette"] = 0.10       # group-clusters silhouette
    r = cmp.compare_step(a, b, 0)
    tol_paths = [d["path"] for d in r["families"]["tolerant"]]
    assert any(p.endswith(".consensus.agree.c0") for p in tol_paths), tol_paths
    assert any("silhouette" in p for p in tol_paths), tol_paths
    assert not r["families"]["exact"], r["families"]["exact"]


def test_group_clusters_id_mismatch_stays_exact():
    # The silhouette rule must NOT reclassify a structural group-clusters id diff.
    cmp = sc.StepComparer()
    a = _blob()
    a["group-clusters"] = [
        {"id": 0, "center": [0.5, 0.5], "members": [1, 2], "silhouette": 0.7}
    ]
    b = copy.deepcopy(a)
    b["group-clusters"][0]["id"] = 9                  # structural (int) divergence
    r = cmp.compare_step(a, b, 0)
    exact_paths = [d["path"] for d in r["families"]["exact"]]
    assert any(p.endswith(".id") for p in exact_paths), r["families"]
...
def test_compare_recordings_rejects_unsafe_engine(bad, tmp_path):
    # engine joins the recording dirs as a path component — reject traversal
    # values before reading anything.
    with pytest.raises(ValueError):
        sc.compare_recordings(tmp_path, tmp_path, engine=bad)
