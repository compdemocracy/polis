...
def test_run_restores_global_logging(tmp_path, monkeypatch):
    """A non-verbose `run` silences logging with logging.disable — it must
    restore it on exit, or one CLI call permanently mutes logging for the rest
    of the process (notably in-process CliRunner test suites)."""
    import logging
    ...
    mod = _module()
    ds = ReplayDataset.build([(10, 0, 1, 1), (11, 1, 1, -1), (12, 0, 2, 1), (13, 1, 2, -1)])
    monkeypatch.setattr(mod, "load_export_votes", lambda slug: ds)
    monkeypatch.setattr(mod, "run_replay", lambda ds, spec, progress=None: [])
    monkeypatch.setattr(mod.st, "write_recording", lambda records, spec, root=None: tmp_path)
    ...
    assert res.exit_code == 0, res.output
    assert logging.root.manager.disable == logging.NOTSET, (
        "logging.disable level leaked past the CLI invocation"
    )
