...
def write_to_dynamodb(dynamodb_client, conversation_id, conv):
    ...
    try:
        ...
        if has_optimized:
            ...
            print(
                f"to_dynamo_dict conversion completed in {conversion_time:.2f}s with {len(dynamo_data)} top-level keys"
            )
...
def connect_to_db():
    ...
    try:
        conn = psycopg2.connect(dbname="polisDB_prod_local_mar14", user="colinmegill", password="", host="localhost")
...
def fetch_votes(conn, conversation_id):  # , limit=0):
    ...
    try:
        ...
        while True:
            ...
            votes.extend(batch)
            print(
                f"[{time.time() - start_time:.2f}s] Fetched batch of {len(batch)} votes, total now: {len(votes)}, batch took {time.time() - fetch_start:.2f}s"
            )
...
def fetch_comments(conn, conversation_id):  # , limit=0):
    ...
    print(
        f"[{time.time() - start_time:.2f}s] Comment stats: {len(comments_list)} usable, {mod_out_count} excluded, {mod_in_count} featured"
    )
...
def fetch_moderation(conn, conversation_id):
    ...
    try:
        ...
        print(
            f"[{time.time() - start_time:.2f}s] Query executed in {time.time() - mod_query_start:.2f}s, fetching results..."
        )
        fetch_start = time.time()
        mod_comments = cursor.fetchall()
        print(
            f"[{time.time() - start_time:.2f}s] Fetched {len(mod_comments)} comment moderation records in {time.time() - fetch_start:.2f}s"
        )
        ...
        if table_exists:
            ...
            print(
                f"[{time.time() - start_time:.2f}s] Query executed in {time.time() - ptpt_query_start:.2f}s, fetching results..."
            )
            ptpt_fetch_start = time.time()
            mod_ptpts = cursor.fetchall()
            print(
                f"[{time.time() - start_time:.2f}s] Fetched {len(mod_ptpts)} participant moderation records in {time.time() - ptpt_fetch_start:.2f}s"
            )
        else:
    ...
    print(
        f"[{time.time() - start_time:.2f}s] Moderation stats: {len(mod_out_tids)} excluded comments, {len(mod_in_tids)} featured comments, {len(meta_tids)} meta comments, {len(mod_out_ptpts)} excluded participants"
    )
...
def get_popular_conversations(conn, limit=5):
    ...
    try:
        ...
        print(
            f"[{time.time() - start_time:.2f}s] Query executed in {time.time() - query_start:.2f}s, fetching results..."
        )
        fetch_start = time.time()
        results = cursor.fetchall()
        print(
            f"[{time.time() - start_time:.2f}s] Found {len(results)} conversations in {time.time() - fetch_start:.2f}s"
        )
...
def test_conversation_from_postgres():
    ...
    try:
        ...
        for idx, (conv_id, vote_count, zinvite) in enumerate(popular_convs):
            ...
            print(
                f"[{time.time() - start_time:.2f}s] Vote retrieval completed in {time.time() - vote_fetch_start:.2f}s - {len(votes['votes'])} votes fetched"
            )
            ...
            print(
                f"[{time.time() - start_time:.2f}s] Moderation summary: {len(moderation['mod_out_tids'])} excluded comments, {len(moderation['mod_in_tids'])} featured comments"
            )

            # Update conversation with votes
            print(
                f"[{time.time() - start_time:.2f}s] Adding {len(votes['votes'])} votes and {len(comments['comments'])} comments to conversation..."
            )
            ...
            try:
                ...
                print(
                    f"[{time.time() - start_time:.2f}s] DynamoDB write {'succeeded' if success else 'failed'} in {time.time() - write_start:.2f}s"
                )
            except Exception as e:
            ...
            assert (
                conv.rating_mat.values.shape[0] == participant_count
            ), "Matrix dimensions should match participant count"
            ...
            if conversations_table:
                ...
                if "Item" in response:
                    ...
                    if math_tick:
                        ...
                        if analysis_table:
                            analysis_response = analysis_table.get_item(Key={"zid": conv_id, "math_tick": math_tick})
                            ...
                            if "Items" in groups_response and groups_response["Items"]:
                                ...
                                if projections_table:
                                    ...
                                    if "Items" in projections_response and projections_response["Items"]:
                                        print("Successfully retrieved participant projections")
                                        print(f"Found {len(projections_response['Items'])} projections")

                    # Basic validation
                    assert (
                        "participant_count" in conversation_item
                    ), "Missing participant_count in conversation metadata"
...
def inspect_dynamodb_data():
    ...
    for item in items:
        print(
            f"  - {item['zid']}: {item.get('participant_count', 0)} participants, "
...
def test_conversation_client_api():
    ...
    config = PostgresConfig(database="polisDB_prod_local_mar14", user="colinmegill", password="", host="localhost")
            ...
            "votes": [
                {
                    ...
                    "vote": v["vote"],
                    "created": (
                        int(float(v["created"]) * 1000)
                        if v["created"] and isinstance(v["created"], str)
                        else (int(v["created"].timestamp() * 1000) if v["created"] else None)
                    ),
                }
                for v in votes
            ]
