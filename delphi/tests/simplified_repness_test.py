...
from simplified_test import kmeans_clustering, load_votes, pca_simple, project_data
...
def calculate_repness(vote_matrix: np.ndarray, clusters: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for _group_idx, group in enumerate(clusters):
        ...
        for comment_idx in range(n_comments):
            ...
            rat = two_prop_test(
                group_comment_stats["pa"],
                group_comment_stats["ns"],
                other_comment_stats["pa"],
                other_comment_stats["ns"],
            )

            rdt = two_prop_test(
                group_comment_stats["pd"],
                group_comment_stats["ns"],
                other_comment_stats["pd"],
                other_comment_stats["ns"],
            )

            # Add to group stats with comparative metrics
            combined_stats = {**group_comment_stats, "ra": ra, "rd": rd, "rat": rat, "rdt": rdt}

            # Calculate agree/disagree metrics
            combined_stats["agree_metric"] = combined_stats["pa"] * (
                abs(combined_stats["pat"]) + abs(combined_stats["rat"])
            )
            combined_stats["disagree_metric"] = combined_stats["pd"] * (
                abs(combined_stats["pdt"]) + abs(combined_stats["rdt"])
            )
            ...
            elif combined_stats["agree_metric"] >= combined_stats["disagree_metric"]:
                combined_stats["repful"] = "agree"
            else:
                combined_stats["repful"] = "disagree"
        ...
        agree_comments = sorted(
            [s for s in group_stats if s["pa"] > s["pd"]], key=lambda s: s["agree_metric"], reverse=True
        )[
            :3
        ]  # Take top 3 agree comments

        disagree_comments = sorted(
            [s for s in group_stats if s["pd"] > s["pa"]], key=lambda s: s["disagree_metric"], reverse=True
        )[
            :2
        ]  # Take top 2 disagree comments
    ...
    try:
        ...
        for group_id, comments in repness_results["group_repness"].items():
            ...
            for i, comment in enumerate(comments):
                ...
                print(
                    f"    Agree metric: {comment['agree_metric']:.2f}, Disagree metric: {comment['disagree_metric']:.2f}"
                )
