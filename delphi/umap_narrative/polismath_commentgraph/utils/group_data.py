...
class GroupDataProcessor:
    ...
    def init_dynamodb(self):
        ...
        try:
            ...
            logger.info(
                f"Successfully initialized DynamoDB connection and accessed table '{self.extremity_table.name}'"
            )
    ...
    def get_math_main_by_conversation(self, zid: int) -> dict[str, Any]:
        ...
        try:
            ...
            if results and "data" in results[0]:
                ...
                try:
                    ...
                    if isinstance(data, dict) and "consensus" in data:
                        consensus_keys = (
                            list(data["consensus"].keys()) if isinstance(data["consensus"], dict) else "not a dict"
                        )
                    ...
                    if isinstance(data, dict) and "participation" in data:
                        ...
                        if isinstance(participation, dict) and "ptptogroup" in participation:
                            ...
                            logger.info(
                                f"Found ptptogroup with type: {type(ptptogroup)} and {len(ptptogroup)} items if dict/list"
                            )
            ...
            math_data = {"group_assignments": group_assignments, "n_groups": 3}  # We created a max of 3 groups above
    ...
    def get_vote_data_by_groups(self, zid: int) -> dict[str, Any]:
        ...
        try:
            ...
            if not group_assignments and isinstance(math_data, dict) and "participation" in math_data:
                ...
                if isinstance(participation, dict) and "ptptogroup" in participation:
                    ...
                    if ptptogroup:
                        ...
                        logger.info("Found group assignments in participation.ptptogroup")
                        ...
                        if len(group_clusters) > 0:
                            ...
                            if isinstance(first_cluster, list) and len(first_cluster) > 0:
                                ...
                                if temp_assignments:
                                    ...
                                    logger.info(
                                        f"Extracted {len(group_assignments)} group assignments from group-clusters list"
                                    )

                            # Check if it's a list of dictionaries (more complex structure)
                            elif isinstance(first_cluster, dict):
                                logger.debug(
                                    f"First cluster keys: {list(first_cluster.keys()) if first_cluster else 'empty'}"
                                )
                                ...
                                if temp_assignments:
                                    group_assignments = temp_assignments
                                    logger.debug(
                                        f"Extracted {len(group_assignments)} group assignments from group-clusters dictionaries"
                                    )
            ...
            if not group_assignments:
                logger.warning(
                    "No group assignments found in math data, generating synthetic groups based on voting patterns"
                )
            ...
            for comment in comments:
                ...
                if tid is not None:
                    vote_data[tid] = {
                        ...
                        "total_disagrees": 0,
                        "total_passes": 0,
                        "groups": defaultdict(lambda: {"votes": 0, "agrees": 0, "disagrees": 0, "passes": 0}),
                    }
            ...
            for tid, data in vote_data.items():
                ...
                for group_id, group_data in groups_data.items():
                    ...
                    group_vote_pcts[group_id] = {"agree": agree_pct, "disagree": disagree_pct, "pass": pass_pct}
                ...
                if len(group_vote_pcts) > 1:
                    ...
                    for i in range(len(group_ids)):
                        for j in range(i + 1, len(group_ids)):
                            ...
                            if group_i != -1 and group_j != -1:
                                ...
                                disagree_diff = abs(
                                    group_vote_pcts[group_i]["disagree"] - group_vote_pcts[group_j]["disagree"]
                                )
                    ...
                    if diffs:
                        ...
                        try:
                            self.store_comment_extremity(zid, tid, avg_diff, "max_vote_diff", component_diffs)
                        except Exception as e:
            ...
            return {"vote_data": {}, "group_assignments": {}, "n_groups": 0}

    def store_comment_extremity(
        self,
        conversation_id: int,
        comment_id: int,
        extremity_value: float,
    ...
    def get_comment_extremity(self, conversation_id: int, comment_id: int) -> dict[str, Any] | None:
        """
    ...
    def get_export_data(self, zid: int, include_moderation: bool) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "math_result": {"group_assignments": group_assignments, "n_groups": group_vote_data["n_groups"]},
            }
            ...
            # Return empty structure to avoid errors downstream
            return {"comments": [], "math_result": {"group_assignments": {}, "n_groups": 0}}
