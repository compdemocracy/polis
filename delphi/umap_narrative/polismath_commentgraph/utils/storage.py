...
class DynamoDBStorage:
    ...
    def get_comment_embedding(self, conversation_id: str, comment_id: int) -> dict[str, Any] | None:
        """
    ...
    def batch_create_comment_embeddings(self, embeddings: list[CommentEmbedding]) -> dict[str, int]:
                ...
                with table.batch_writer() as writer:
                    for embedding in batch:
                        try:
                            ...
                            logger.error(f"Error processing embedding for comment {embedding.comment_id}: {str(e)}")
                            failure_count += 1
            except ClientError as e:
        ...
        logger.info(f"Batch created {success_count} comment embeddings with {failure_count} failures")
    ...
    def batch_create_comment_clusters(self, clusters: list[CommentCluster]) -> dict[str, int]:
                ...
                with table.batch_writer() as writer:
                    for cluster in batch:
                        try:
                            ...
                            logger.error(f"Error processing cluster for comment {cluster.comment_id}: {str(e)}")
                            failure_count += 1
            except ClientError as e:
                logger.error(f"Error in batch write operation: {str(e)}")
                # Count all items in this batch as failures
        ...
        logger.info(f"Batch created {success_count} comment clusters with {failure_count} failures")
    ...
    def batch_create_cluster_topics(self, topics: list[ClusterTopic]) -> dict[str, int]:
                ...
                with table.batch_writer() as writer:
                    for topic in batch:
                        try:
                            ...
                            logger.error(f"Error processing topic for cluster {topic.cluster_key}: {str(e)}")
                            failure_count += 1
            except ClientError as e:
                logger.error(f"Error in batch write operation: {str(e)}")
                # Count all items in this batch as failures
        ...
        logger.info(f"Batch created {success_count} cluster topics with {failure_count} failures")

        return {"success": success_count, "failure": failure_count}

    def get_cluster_topics_by_layer(self, conversation_id: str, layer_id: int) -> list[dict[str, Any]]:
        """
    ...
    def batch_create_cluster_characteristics(self, characteristics):
        ...
        logger.info(f"Batch created {success_count} cluster characteristics with {failure_count} failures")
    ...
    def batch_create_enhanced_topic_names(self, topic_names):
                ...
                with table.batch_writer() as writer:
                    for topic_name in batch:
                        try:
                            ...
                            logger.error(f"Error processing enhanced topic name for {topic_name.topic_key}: {str(e)}")
                            failure_count += 1
            except ClientError as e:
                logger.error(f"Error in batch write operation: {str(e)}")
                # Count all items in this batch as failures
        ...
        logger.info(f"Batch created {success_count} enhanced topic names with {failure_count} failures")
    ...
    def batch_create_llm_topic_names(self, topic_names):
                ...
                with table.batch_writer() as writer:
                    for topic_name in batch:
                        try:
                            ...
                            logger.error(f"Error processing LLM topic name for {topic_name.topic_key}: {str(e)}")
                            failure_count += 1
            except ClientError as e:
                logger.error(f"Error in batch write operation: {str(e)}")
                # Count all items in this batch as failures
        ...
        logger.info(f"Batch created {success_count} LLM topic names with {failure_count} failures")
    ...
    def batch_create_graph_edges(self, edges: list[UMAPGraphEdge]) -> dict[str, int]:
                ...
                with table.batch_writer() as writer:
                    for edge in batch:
                        try:
                            ...
                            logger.error(f"Error processing edge {edge.edge_id}: {str(e)}")
                            failure_count += 1
            except ClientError as e:
                logger.error(f"Error in batch write operation: {str(e)}")
                # Count all items in this batch as failures
        ...
        logger.info(f"Batch created {success_count} graph edges with {failure_count} failures")

        return {"success": success_count, "failure": failure_count}

    def get_visualization_data(self, conversation_id: str, layer_id: int) -> dict[str, Any]:
        """
        ...
        try:
            ...
            response = clusters_table.query(KeyConditionExpression=Key("conversation_id").eq(conversation_id))
            ...
            for comment in comments:
                ...
                cluster_info = next((c for c in clusters if c["comment_id"] == comment["comment_id"]), None)
            ...
            return {"conversation_id": conversation_id, "layer_id": layer_id, "comments": [], "clusters": []}
