...
class Config:
    ...
    def _get_defaults(self) -> Dict[str, Any]:
        ...
        return {
            ...
            # Database
            "database": {"pool-size": 5, "max-overflow": 10},
            # Polling
            "poller": {
    ...
    def _apply_env_vars(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Server
        config["server"]["port"] = to_int(
            os.environ.get("PORT", config["server"]["port"])
        )
        config["server"]["host"] = os.environ.get("HOST", config["server"]["host"])

        # Database
        config["database"]["pool-size"] = to_int(
            os.environ.get("DATABASE_POOL_SIZE", config["database"]["pool-size"])
        )
        config["database"]["max-overflow"] = to_int(
            os.environ.get("DATABASE_MAX_OVERFLOW", config["database"]["max-overflow"])
        )

        # Polling
        config["poller"]["vote-interval"] = (
            to_float(
                os.environ.get(
                    "POLL_VOTE_INTERVAL_MS",
                    to_float(
                        os.environ.get(
                            "POLL_INTERVAL_MS", config["poller"]["vote-interval"] * 1000
                        )
                    ),
                )
            )
        ...
        config["poller"]["mod-interval"] = (
            to_float(
                os.environ.get(
                    "POLL_MOD_INTERVAL_MS",
                    to_float(
                        os.environ.get(
                            "POLL_INTERVAL_MS", config["poller"]["mod-interval"] * 1000
                        )
                    ),
                )
            )
        ...
        config["poller"]["task-interval"] = (
            to_float(
                os.environ.get(
                    "POLL_TASK_INTERVAL_MS",
                    to_float(
                        os.environ.get(
                            "POLL_INTERVAL_MS", config["poller"]["task-interval"] * 1000
                        )
                    ),
                )
            )
        ...
        config["poller"]["allowlist"] = to_int_list(
            os.environ.get("POLL_ALLOWLIST", [])
        )
        config["poller"]["blocklist"] = to_int_list(
            os.environ.get("POLL_BLOCKLIST", [])
        )

        # Conversation
        config["conversation"]["max-ptpts"] = to_int(
            os.environ.get("CONV_MAX_PTPTS", config["conversation"]["max-ptpts"])
        )
        config["conversation"]["max-cmts"] = to_int(
            os.environ.get("CONV_MAX_CMTS", config["conversation"]["max-cmts"])
        )
        config["conversation"]["group-k-min"] = to_int(
            os.environ.get("CONV_GROUP_K_MIN", config["conversation"]["group-k-min"])
        )
        config["conversation"]["group-k-max"] = to_int(
            os.environ.get("CONV_GROUP_K_MAX", config["conversation"]["group-k-max"])
        )
