...
def two_prop_test(success_count_1: int, total_count_1: int, success_count_2: int, total_count_2: int) -> float:
    """
    ...
    pooled_p_hat = min(0.9999, pooled_p_hat)

    # Standard error
    se = math.sqrt(pooled_p_hat * (1 - pooled_p_hat) * (1 / total_count_1_adj + 1 / total_count_2_adj))
...
def weighted_stddev(values: np.ndarray, weights: np.ndarray | None = None) -> float:
    """
...
def bootstrap_ci_95(values: np.ndarray, statistic: callable = np.mean, n_bootstrap: int = 1000) -> tuple[float, float]:
    """
