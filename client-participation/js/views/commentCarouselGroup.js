...
module.exports = CommentCarousel.extend({
  ...
  generateItemsHTML: function () {
    var that = this;
    var selectedGroupName = _.isUndefined(that.selectedGid) ? "" : Utils.getGroupNameForGid(that.selectedGid);
  ...
  initialize: function (options) {
    ...
    eb.on(eb.clusterSelectionChanged, function (gid) {
      selectedGroupName = _.isUndefined(that.selectedGid) ? "" : Utils.getGroupNameForGid(that.selectedGid);
    ...
    function doFetch(gid) {
      that.collection.firstFetchPromise.then(function () {
        if (gid >= 0) {
          getTidsForGroup(gid, that.commentLimit).then((o) => {
