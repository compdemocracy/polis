...
const BarChart = ({ selectedComment, groupVotes, groups, translate }) => {
  ...
  if (groups) {
    ...
    commentPassCount = _.reduce(
      groups,
      (accumulator, group, key) => {
        return (accumulator +=
          group.votes[selectedComment.tid].S -
          (group.votes[selectedComment.tid].A + group.votes[selectedComment.tid].D));
      },
      0
    );
  }
