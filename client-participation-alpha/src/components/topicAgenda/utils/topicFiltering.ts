...
import type { UmapPoint, FilteredTopic } from '../types'
...
export const getFilteredTopics = (
  ...
  layerId: number,
  hierarchyAnalysis: { layers: number[] } | null,
  bankedTopics: Map<number, Set<string>>,
  ...
  const topicsWithProximity = Object.entries(allTopics).map(([clusterId, topic]): FilteredTopic => {
    const clusterKey = `${layerId}_${clusterId}`
    const targetPoints = clusterGroups[layerId].get(clusterKey)

    let minProximity = Infinity
    let closestBankedTopic: string | null = null
    ...
    if (targetPoints && targetPoints.length > 0) {
      ...
      if (targetCentroid) {
        ...
        bankedFromHigherLayer.forEach((bankedTopicKey) => {
          ...
          let bankedClusterId: string | undefined
  ...
  let filteredTopics: FilteredTopic[]
  if (layerId === maxLayer - 1) {
    ...
    filteredTopics = topicsWithProximity.filter((item) => item.source === 'close')
  }
