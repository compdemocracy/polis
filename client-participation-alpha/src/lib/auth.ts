...
export function setJwtToken(token: string) {
  ...
  try {
    ...
    if (!conversationId) {
      console.error(
        '[PolisStorage] No conversation_id in JWT, cannot store participant token securely.'
      )
      return
    }
    ...
    if (window.localStorage) {
      ...
      window.dispatchEvent(
        new CustomEvent('polis-token-update', {
    ...
    console.error('[PolisStorage] Error storing JWT token')
  }
}
 ...
 */
export function handleJwtFromResponse(response: AuthResponse | unknown): void {
  if (typeof window === 'undefined') {
    return
  }

  // Use type narrowing to check if response matches expected structure
  const authResponse = response as AuthResponse
  if (authResponse && authResponse.auth && authResponse.auth.token) {
    console.log('[Auth] JWT token found in response, storing...')
    setJwtToken(authResponse.auth.token)
  }
}
